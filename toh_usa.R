...
roads.d <- raster('C:\\Users\\bjselige\\Documents\\roads.distance.tif')
...
#write.csv(data.frame(myBiomodModelEval[c('TSS'),"Testing.data",,,]), 'C:\\Users\\bjselige\\Desktop\\toh_usa_eval.csv')
...
values(pred.out) <- scales::rescale(values(pred.out), to=c(0,1),
                                    from=c(min(values(pred.out), na.rm=T),
                                           max(values(pred.out), na.rm=T)))
...
eval.df <-  data.frame(myBiomodModelEval[c('TSS'),"Testing.data",,,]) # print the eval scores of all selected models

h <- hist(raster::extract(pred.out, aa.pts), breaks=20)
plot(h$breaks[2:21], (h$counts/12251), type='l', xlab='predicted value', ylab='percentage of presences')



v1 <- data.frame(v1=raster::extract(pred.out, aa.pts))

ggeval <- ggplot(data=v1, aes('v1') + geom_histogram(binwidth = 1)
